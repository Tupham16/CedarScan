...
extension LegalDoc {
    static let termsSectionsFR: [(String, String)] = [
        ...
        ("Paiement",
         "Les commandes payantes sont réglées par carte par l'intermédiaire de Stripe, notre prestataire de paiement : dans l'application, lorsque celle-ci le propose, ou, dans votre navigateur, sur notre page de paiement, qui peut aussi proposer d'autres modes de paiement. Le montant débité est le total affiché sur le formulaire de paiement, en dollars américains, après application de tout code promotionnel. Si vous choisissez d'enregistrer une carte dans l'application, elle est conservée par Stripe, non par nous, et n'est utilisée que lorsque vous confirmez un paiement ultérieur ; vous pouvez la retirer dans le formulaire de carte de l'application chaque fois qu'il s'ouvre pour un paiement ultérieur, ou à tout moment en nous écrivant ou en supprimant votre compte. La production commence une fois le paiement confirmé, sauf si la commande est couverte par le quota de commandes gratuites, auquel cas elle commence immédiatement. Si un paiement est annulé ou fait l'objet d'une rétrofacturation, nous pouvons suspendre la commande ou retenir les livrables jusqu'à ce que la situation soit résolue."),
