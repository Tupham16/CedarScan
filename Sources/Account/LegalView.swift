...
extension LegalDoc {
    static let privacySections: [(String, String)] = [
        ...
        ("How long we keep it",
         "On your phone, scans stay until you delete them — nothing is removed on a timer. Deleting a property in the app deletes that property's scans from the phone, and that cannot be undone; any copy already uploaded to us — for an order, or when you opened the order form from that scan without ordering — is not affected. Each scan is large, so delete the properties you have finished with to reclaim space. On our side we currently keep scan files, order attachments and order records for as long as they are needed for the order and its records; a delivered drawing can come back for revision, and order records are required for tax and accounting. We do not run an automatic purge of these files today. Finished drawings stay available for you to download for at least 365 days from delivery; after that we may remove them, so keep your own copy. Order records, order attachments and the files of any order still open when you delete your account outlive the account itself — see \"Deleting your account\" below for exactly what goes and what stays. If you want your scan files removed sooner, delete your account; for the files of an order still in progress, and for order attachments, email us and we will remove them once the order is closed."),
        ("Deleting your account",
         "Account, Delete account removes your account and, from our storage, your scan files and any virtual tour photos. We also ask Stripe to delete any customer record the app created for you at Stripe, together with any card you saved in the app; Stripe keeps its own records of the payments themselves, which we can still see in our Stripe account. Deleting the account does not affect a card saved on our payment page: one saved to a cedar247.com account can be removed from that account or by writing to us, and one saved with Stripe's Link stays in your own Link account with Stripe, which you manage yourself. On our side, besides the device record described under \"Device identifier\", the following deliberately survive the deletion. First, orders already placed stay in our business records: the order number, date and amount, the packages and options you chose and any coupon used, how and when the order was paid, with the reference that links it to the payment's record (at Stripe where the app prepared Stripe's card form for it, or on our website where you paid on our payment page), the name and email that were on the order, the property address or label you gave it, your order notes, the revision requests and messages you sent about it, the names and measurements of the scans that came with it, any virtual tour's title and room labels, and the list of files that came with it — tax law requires the record, and a delivered drawing can still come back for revision. Second, the scan files and virtual tour photos of any order still open when you delete the account, paid or not, are kept so that the job can be finished; they are not removed automatically when the order closes, so email us once it is closed and we will delete them. Third, the other files you uploaded yourself — such as a logo, or a photo or marked-up PDF for a revision request — stay on our storage: those sent with an order stay attached to it, and one you uploaded but did not send in the end stays too; if you want such an unsent upload removed, tell us before you delete your account. The drawings we delivered also stay available at their links (see \"How long we keep it\"). A virtual tour page we published for you stays online at its link with its title, often the property address, and anything our team added to it; the photos you uploaded for it, and any floor plan taken from your scans, are deleted with the account unless the order is still open, so the tour will no longer show them — email us if you want the page taken down. Your account record itself — name, email address, password hash and the rest of it — and the records of your scans remain in copies of our database: the daily backups, deleted about 60 days later, and occasional copies we make by hand for safekeeping, kept for as long as we need them. Our ordinary technical records (see \"Technical records\") also outlive the account for as long as we keep them to run the service. Email \(contactEmail) if you want those files and the personal details on closed orders removed, and we will do it wherever the law lets us. Deletion of the account itself cannot be undone."),
        ("Your rights",
...
extension LegalDoc {
    static let privacySectionsFR: [(String, String)] = [
        ...
        ("Combien de temps nous les conservons",
         "Sur votre téléphone, les numérisations restent jusqu'à ce que vous les supprimiez — rien n'est supprimé automatiquement au bout d'un délai. Supprimer une propriété dans l'application supprime du téléphone les numérisations de cette propriété, et cela ne peut pas être annulé ; toute copie déjà téléversée chez nous — pour une commande, ou lorsque vous avez ouvert le formulaire de commande depuis cette numérisation sans commander — n'est pas concernée. Chaque numérisation est volumineuse ; supprimez donc les propriétés dont vous n'avez plus besoin pour récupérer de l'espace. De notre côté, nous conservons actuellement les fichiers de numérisation, les pièces jointes de commande et les dossiers de commande aussi longtemps qu'ils sont nécessaires à la commande et à ses registres ; un plan livré peut revenir pour révision, et les dossiers de commande sont exigés à des fins fiscales et comptables. Nous n'effectuons pas aujourd'hui de purge automatique de ces fichiers. Les plans terminés restent disponibles en téléchargement pendant au moins 365 jours à compter de la livraison ; passé ce délai, nous pouvons les supprimer — conservez donc votre propre copie. Les dossiers de commande, les pièces jointes de commande et les fichiers de toute commande encore ouverte au moment de la suppression du compte survivent au compte lui-même — voyez \"Suppression de votre compte\" ci-dessous pour savoir exactement ce qui disparaît et ce qui reste. Si vous voulez que vos fichiers de numérisation soient supprimés plus tôt, supprimez votre compte ; pour les fichiers d'une commande encore en cours, comme pour les pièces jointes de commande, écrivez-nous par courriel et nous les supprimerons une fois la commande clôturée."),
        ("Suppression de votre compte",
         "Dans Compte, l'option Supprimer le compte supprime votre compte et, de notre stockage, vos fichiers de numérisation ainsi que vos éventuelles photos de visite virtuelle. Nous demandons aussi à Stripe de supprimer toute fiche client que l'application a créée pour vous chez Stripe, avec toute carte que vous avez enregistrée dans l'application ; Stripe conserve ses propres registres des paiements eux-mêmes, que nous pouvons encore voir dans notre compte Stripe. La suppression du compte ne touche pas une carte enregistrée sur notre page de paiement : une carte enregistrée dans un compte cedar247.com peut être retirée depuis ce compte, ou par nous si vous nous écrivez, et une carte enregistrée avec Link de Stripe reste dans votre propre compte Link chez Stripe, que vous gérez vous-même. De notre côté, outre la trace décrite sous \"Identifiant de l'appareil\", les éléments suivants survivent délibérément à la suppression. Premièrement, les commandes déjà passées restent dans nos registres commerciaux : le numéro, la date et le montant de la commande, les forfaits et options choisis et tout code promotionnel utilisé, le mode et la date du paiement de la commande, avec la référence qui la relie au dossier de ce paiement (chez Stripe lorsque l'application a préparé pour elle le formulaire de carte de Stripe, ou sur notre site web lorsque vous avez payé sur notre page de paiement), le nom et l'adresse courriel figurant sur la commande, l'adresse ou le libellé de la propriété que vous lui avez donné, vos notes de commande, les demandes de révision et les messages que vous avez envoyés à son sujet, les noms et les mesures des numérisations qui l'accompagnaient, le titre et les libellés de pièces de toute visite virtuelle, et la liste des fichiers qui l'accompagnaient — le droit fiscal exige ce registre, et un plan livré peut encore revenir pour révision. Deuxièmement, les fichiers de numérisation et les photos de visite virtuelle de toute commande encore ouverte au moment de la suppression, payée ou non, sont conservés pour permettre d'achever le travail ; ils ne sont pas supprimés automatiquement à la clôture de la commande : écrivez-nous une fois celle-ci clôturée et nous les supprimerons. Troisièmement, les autres fichiers que vous avez vous-même téléversés — par exemple un logo, ou une photo ou un PDF annoté pour une demande de révision — restent sur notre stockage : ceux envoyés avec une commande y restent attachés, et un fichier téléversé mais finalement non envoyé reste lui aussi ; si vous voulez qu'un tel fichier non envoyé soit supprimé, dites-le-nous avant de supprimer votre compte. Les plans que nous avons livrés restent aussi accessibles par leurs liens (voyez \"Combien de temps nous les conservons\"). Une page de visite virtuelle publiée pour vous reste en ligne à son adresse, avec son titre — souvent l'adresse de la propriété — et ce que notre équipe y a ajouté ; les photos que vous avez téléversées pour elle, et tout plan tiré de vos numérisations, sont supprimés avec le compte sauf si la commande est encore ouverte, de sorte que la visite ne les affichera plus — écrivez-nous si vous voulez que la page soit retirée. La fiche de votre compte elle-même — nom, adresse courriel, empreinte cryptographique du mot de passe et le reste — ainsi que les fiches de vos numérisations demeurent dans des copies de notre base de données : les sauvegardes quotidiennes, supprimées environ 60 jours plus tard, et les copies que nous faisons à la main à l'occasion pour les mettre en sûreté, conservées aussi longtemps que nous en avons besoin. Nos journaux techniques ordinaires (voyez \"Journaux techniques\") survivent aussi au compte, pendant toute la durée où nous les conservons pour faire fonctionner le service. Écrivez à \(contactEmail) si vous souhaitez que ces fichiers et les renseignements personnels figurant sur les commandes clôturées soient supprimés, et nous le ferons partout où la loi nous le permet. La suppression du compte lui-même ne peut pas être annulée."),
        ("Vos droits",
