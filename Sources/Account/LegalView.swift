...
extension LegalDoc {
    static let privacySectionsFR: [(String, String)] = [
        ...
        ("Pourquoi nous utilisons vos renseignements",
         "Pour exécuter notre contrat avec vous : produire et livrer les plans que vous commandez, encaisser le paiement et vous assister ensuite. Pour nos intérêts légitimes : maintenir la sécurité et le fonctionnement du service, tester ses modifications, prévenir les abus des promotions, améliorer la précision des consignes de capture et comprendre comment le service est utilisé, par exemple qui s'inscrit, qui commande et qui s'arrête en chemin sans commander. Pour respecter nos obligations légales : fiscalité, comptabilité et tenue de registres. Lorsque la loi exige un consentement — par exemple pour la localisation de l'appareil — nous le demandons, et vous pouvez le refuser ou le retirer dans les réglages de votre appareil."),
