...
enum LegalDoc: String, CaseIterable, Identifiable {
    ...
    /// (18/09/2026: in-app card payment via Stripe (PaymentSheet) — Stripe named as the payment
    /// provider in Privacy "Information you give us" / "Who sees your data" / "Deleting your
    /// account", Terms "Payment", EULA "Third-party services"; EN + FR. Worded to stay true whether
    /// the in-app channel is switched on or off. `date`: 2026-09-18. Review round 1: "Who sees your
    /// data" now names what Stripe's form reports (usage + device details) and what we see in our
    /// Stripe account but do not store. Rounds 2-3: Stripe's own purposes and every list non-exhaustive.
    /// Round 4: the precise claims are scoped to payments made in the app; the browser payment page is
    /// WooCommerce on cedar247.com, which can send Stripe more and keep saved-card records itself.
    /// Round 5: data reaches Stripe when the app PREPARES the card form (it opens by itself after
    /// Place order), not only when the customer pays; Stripe's SDK also sends the iOS vendor
    /// identifier ("Device identifier"); account deletion removes the in-app Stripe customer.
    /// Round 6: Stripe is held to the same protection standard (5.1.1(i)) but not to "our
    /// instructions only"; the device record is named among what survives deletion.
    /// Round 7: "all providers bound to the standard" + only hosting/storage/email "on our instructions only";
    /// IP address listed for Stripe's form; deletion list matches account/delete/route.ts (files of
    /// every OPEN order kept, not purged at close; payment reference kept; technical records outlive).
    /// Round 8: "How long we keep it" agrees with that; the deletion list also names unsent uploads,
    /// delivered drawings / tour pages at their links, and the 60-day database backups (db-backup.ts).
    /// Round 9: tour pages stay online but lose the customer's photos (tours/ of closed orders are
    /// deleted); hand-made database copies disclosed; unsent uploads: ask before deleting.
    /// Round 10: "Deleting your account" rewritten as a NON-exhaustive, category-based paragraph (every
    /// attempt at an exact list missed something: dev/test DB copy, internal reports, the website
    /// order of each paid order, tour room names); "Why we use your data" names testing.
    /// Round 11: removal promise limited to files/pages/order details (DB copies + logs are not edited
    /// one by one); scan-based tour plans go with the scans; "internal reporting" NOT named as a purpose
    /// (it would be Apple's Analytics purpose on the App Privacy label — an owner decision, see plan §0e;
    /// SUPERSEDED by the 20/09 entry below: the owner chose to declare it).
    /// Round 12: photos the team added to a tour for the customer (tour/<orderId>/) stay on the page.
    /// Round 13: only the automatic plan image from the scan (scans/<cid>/) goes; plans the team drew or
    /// uploaded (tour/<orderId>/) stay.
    /// Round 14: that image is named by where it is stored ("came with your scan files"), not by how it
    /// was made, so a copy the team uploads to the tour is clearly on the kept side.
    /// Round 15: the FR removal request is plural ("des fichiers … soient supprimés"), like the EN "any of".
    /// 20/09: owner decision (E), reversing round 11 — the hand-run customer report analyses app accounts
    /// AND their scan rows, so "Why we use your data" now names understanding how the service is used,
    /// "Deleting your account" names that purpose on the smaller hand-made extracts (see round 23 below),
    /// and the manifest + App Privacy table
    /// give seven types Apple's Analytics purpose (email, name, property address, purchase history, account
    /// id, other user content = the scan label, other usage data). Round 22: the scan name is a floor label, not the
    /// address, so the property address keeps App Functionality only. Round 23: the hand-made copies hold the
    /// same records as the database (`backup.mjs` dumps every table), so they stay in the sentence that names
    /// the account record, and the reporting extract is described on its own.
    /// Round 24: the property address DOES reach that report — the app puts it in the WooCommerce order
    /// description and the report prints the line items — so Physical Address carries Analytics again; the
    /// extract sentence names the scan records too, and the extracts join the "not edited one by one" list.)
    /// (25/09/2026: Orders v2 B "unpaid = not placed", owner approved the wording: "Who sees your data"
    /// no longer names the "Pay Now" label (awaiting orders say "Pay $X") nor "place a paid order";
    ...
    /// `date`: 2026-09-26.)
    /// (27/09/2026: push notifications (2.61, owner approved 26/09): new section "Notifications" after
    /// "Device identifier" (owner's wording) + "Who sees your data" names Apple's push service as the
    /// carrier (token + notification content). App Store table unchanged (Device ID, App
    /// Functionality, linked). `date`: 2026-09-27.)
    static let lastUpdated = "27 September 2026"

    /// Ngày cập nhật viết theo lối Pháp, in trên bản Pháp. Đổi `lastUpdated` thì đổi cả đây.
    static let lastUpdatedFR = "27 septembre 2026"
...
extension LegalDoc {
    static let privacySections: [(String, String)] = [
        ...
        ("Device identifier",
         "The app reads the identifier iOS gives to apps from the same developer on your device, and sends it to us when you create your account, when you view prices and when you place an order. We use it for one purpose: to apply the limited free-order allowance per device as well as per account. Where the app shows Stripe's card form itself, that form also sends this identifier to Stripe (see \"Who sees your data\"). It is not the advertising identifier and it is not shared with third parties for marketing. iOS resets it once you remove every Cedar247 app from the device. We keep a record of the identifier and how many free orders it has used even after an account is deleted, otherwise the allowance could be reset simply by making a new account; write to us if you want that record removed."),
        ("Notifications",
         "If you allow notifications, iOS gives the app a push token for your device, and the app sends it to us so we can tell you about your orders: when drawings are ready, when a payment is due or an unpaid order was cancelled, and when our team has written to you. We keep the token with your account until you sign out, delete your account or the token stops working. Each notification names the property label or the order number and is delivered through Apple's push notification service. You can turn notifications off at any time in your device settings."),
        ("Technical records",
        ...
        ("Why we use your data",
         "To perform our contract with you: producing and delivering the drawings you order, taking payment, and supporting you afterwards. For our legitimate interests: keeping the service secure and running, testing changes to it, preventing abuse of promotions, improving the accuracy of the capture guidance, and understanding how the service is used, for example who registers, who orders, and who stops before ordering. To meet legal obligations: tax, accounting and record-keeping. Where the law requires consent — for example device location — we ask for it and you may refuse or withdraw it in your device settings."),
        ("Who sees your data",
         "The Cedar247 production team and the drafting specialists working on your order see the scan files, the property label and your order notes, under confidentiality obligations. We also use service providers who process data on our behalf: cloud hosting and object storage, an email delivery provider, and Stripe, our payment provider. All of them are bound by terms that require them to protect your data to at least the standard set out in this policy. Our hosting, storage and email providers act on our instructions only and use your data only for the service they provide to us. Stripe does not act only on our instructions: as a payment company it also uses payment data for its own purposes, such as fraud prevention and security, meeting its own legal obligations and improving its services. Its payment form reports to Stripe how the form is used, together with your network (IP) address and basic details of your device such as its model, system version, screen size, language and time zone and identifiers Stripe generates, and, in the app, the app version and the identifier iOS gives to apps from the same developer (see \"Device identifier\"); Stripe uses this for its own analytics and fraud prevention. Stripe handles all of this under its own privacy policy. When you pay by card, whether in the app or on our payment page, you give your card details to Stripe directly: they never pass through our servers and we never see your full card number. When the app prepares Stripe's card form for an order, we pass Stripe your name, email address, identifiers for your account and your order, the order number and the amount due, whether or not you then pay there; where the app offers that form, the app prepares it by itself right after you tap \"Place order\" for an order that is not free, and again each time you tap a button to pay for it. Once that has happened, Stripe keeps your name and email address in a customer record under our Stripe account, together with any card you choose to save in the app; we ask Stripe to delete that record when you delete your account. We receive back the result and the amount of any payment. In our Stripe account we can also see the record Stripe keeps of the card — including its brand, last four digits, expiry date, type and country, and the postal code if the form asks for one — and Stripe's own fraud assessment of the payment; for payments made in the app, we do not store that record or that assessment in our own systems. Our payment page is part of our website, cedar247.com, and runs on its shop software: it may pass Stripe more about you than the app does — for example the details of a cedar247.com account you are signed in to, such as its billing address — and the website may keep records of those payments, such as the brand, last four digits and expiry date of a card you save to that account. If you choose another payment method offered on our payment page, the provider of that method handles your details under its own privacy policy. If you allow notifications, they reach your device through Apple's push notification service: Apple receives the push token and the content of each notification, which names the property label or the order number (see \"Notifications\"), and handles them under its own privacy policy. We may disclose data if the law requires it, or to establish or defend legal claims. We do not sell personal data and we do not share it with data brokers."),
        ...
        ("How long we keep it",
         "On your phone, scans stay until you delete them — nothing is removed on a timer. Deleting a property in the app deletes that property's scans from the phone, and that cannot be undone; any copy already uploaded to us — for an order, or when you opened the order form from that scan without ordering — is not affected. Each scan is large, so delete the properties you have finished with to reclaim space. On our side we currently keep scan files, order attachments and order records for as long as they are needed for the order and its records; a delivered drawing can come back for revision, and order records are required for tax and accounting. A scan uploaded when you opened the order form without ordering, or whose order was cancelled before it was paid, stays until you delete your account or ask us to remove it. We do not run an automatic purge of these files today. Finished drawings stay available for you to download for at least 365 days from delivery; after that we may remove them, so keep your own copy. Some of this outlives the account itself — see \"Deleting your account\" below. If you want your scan files removed sooner, delete your account; for the files of an order still in progress, and for order attachments, email us and we will remove them once the order is closed."),
        ("Deleting your account",
         "Account, Delete account removes your account and, from our storage, your scan files and the virtual tour photos you uploaded in the app, except those described below. We also ask Stripe to delete any customer record the app created for you at Stripe, together with any card you saved in the app; Stripe keeps its own records of the payments themselves, which we can still see in our Stripe account. Deleting the account does not affect a card saved on our payment page: one saved to a cedar247.com account can be removed from that account or by writing to us, and one saved with Stripe's Link stays in your own Link account with Stripe, which you manage yourself. Some data survives the deletion, mainly the following. Our business records of the orders you placed stay, together with the order our website creates for each paid order, in which its payment is recorded; they hold what you gave us for each order, such as your name and email, the property address or label, the packages, options and coupon, your notes, revision requests and messages, the names and measurements of your scans and the payment references, because tax law requires the record and a delivered drawing can still come back for revision. An order placed in the app that is still awaiting payment is cancelled first, so its scan files and tour photos are removed like the rest, unless it cannot be cancelled at that moment, for example because a payment for it is under way. The scan files and virtual tour photos of any other order still open when you delete the account, paid or not, stay so that the job can be finished, and they are not removed automatically when it closes. Other files you attached yourself, such as a logo or a marked-up PDF, stay whether or not you sent them with an order. The drawings we delivered, and any virtual tour page we published, stay at their links; a tour page loses the photos you uploaded in the app, and any floor plan image that came with your scan files, unless its order is still open, but keeps its title (often the property address), the room and floor names, the floor plans our team drew or uploaded, any photos you sent us that our team added to it for you, and anything else our team added. The device record described under \"Device identifier\" stays too. Copies of our database also keep your account record, including your password hash, and your scan records: the daily backups, deleted about 60 days later, and the copies we make by hand from time to time to keep data safe or to test changes to the service, kept for as long as we need them. We also take smaller extracts by hand — account and order details, and your scan records with the label you typed, the type, the status and the dates — to understand how the service is used, and keep those for as long as we need them. Our ordinary technical records (see \"Technical records\") stay for as long as we keep them to run the service. Email \(contactEmail) if you want any of the files, pages or order details above removed — for example the files of an order once it is closed, an attachment you never sent, or a tour page — and we will do it wherever the law lets us; database copies, those extracts and technical records are not edited one by one, and Stripe's own records are governed by Stripe's privacy policy. Deletion of the account itself cannot be undone."),
        ("Your rights",
...
extension LegalDoc {
    static let termsSections: [(String, String)] = [
        ...
        ("Payment",
         "Paid orders are settled either by card through Stripe, our payment provider (in the app where the app offers it, or on our payment page in your browser), or by any other payment method that page offers. The amount charged is the total shown on the payment form, in United States dollars, after any coupon has been applied. If you choose to save a card in the app, it is stored by Stripe, not by us, and used only when you confirm a later payment; you can remove it in the app's card form whenever that form opens for a later payment, or at any time by writing to us or by deleting your account, which asks Stripe to delete it. Production starts once payment is confirmed, unless the order is covered by a free allowance, in which case it starts immediately. If a payment is reversed or charged back, we may pause the order or withhold the deliverables until the matter is resolved."),
        ("Turnaround",
...
extension LegalDoc {
    static let eulaSections: [(String, String)] = [
        ...
        ("Third-party services",
         "The app relies on services we do not control, including Apple's platform frameworks, our cloud hosting and storage providers, and Stripe, our payment provider, which supplies the card payment form. Their terms apply to their part of the service, and we are not responsible for how those third parties operate. You agree to comply with any third-party terms that apply to your use of the app."),
        ("No warranty",
...
extension LegalDoc {
    static let privacySectionsFR: [(String, String)] = [
        ...
        ("Identifiant de l'appareil",
         "L'application lit l'identifiant qu'iOS attribue, sur votre appareil, aux applications provenant du même développeur, et nous le transmet lorsque vous créez votre compte, lorsque vous consultez les prix et lorsque vous passez une commande. Nous ne l'utilisons que dans un seul but : appliquer le quota limité de commandes gratuites non seulement par compte, mais aussi par appareil. Lorsque l'application affiche elle-même le formulaire de carte de Stripe, ce formulaire transmet aussi cet identifiant à Stripe (voyez \"Qui voit vos renseignements\"). Il ne s'agit pas de l'identifiant publicitaire et il n'est pas communiqué à des tiers à des fins de marketing. iOS le réinitialise dès que vous supprimez toutes les applications Cedar247 de l'appareil. Nous conservons une trace de l'identifiant et du nombre de commandes gratuites qu'il a utilisées même après la suppression d'un compte, faute de quoi le quota pourrait être remis à zéro par la simple création d'un nouveau compte ; écrivez-nous si vous souhaitez que cette trace soit supprimée."),
        ("Notifications",
         "Si vous autorisez les notifications, iOS remet à l'application un jeton de notification propre à votre appareil, et l'application nous le transmet afin que nous puissions vous informer au sujet de vos commandes : lorsque des plans sont prêts, lorsqu'un paiement est dû ou qu'une commande impayée a été annulée, et lorsque notre équipe vous a écrit. Nous conservons le jeton avec votre compte jusqu'à ce que vous vous déconnectiez, que vous supprimiez votre compte ou que le jeton cesse de fonctionner. Chaque notification indique le libellé de la propriété ou le numéro de commande et est acheminée par le service de notifications poussées d'Apple. Vous pouvez désactiver les notifications à tout moment dans les réglages de votre appareil."),
        ("Journaux techniques",
        ...
        ("Pourquoi nous utilisons vos renseignements",
         "Pour exécuter notre contrat avec vous : produire et livrer les plans que vous commandez, encaisser le paiement et vous assister ensuite. Pour nos intérêts légitimes : maintenir la sécurité et le fonctionnement du service, tester ses modifications, prévenir les abus des promotions, améliorer la précision des consignes de capture et comprendre comment le service est utilisé, par exemple qui s'inscrit, qui commande et qui s'arrête avant de commander. Pour respecter nos obligations légales : fiscalité, comptabilité et tenue de registres. Lorsque la loi exige un consentement — par exemple pour la localisation de l'appareil — nous le demandons, et vous pouvez le refuser ou le retirer dans les réglages de votre appareil."),
        ("Qui voit vos renseignements",
         "L'équipe de production de Cedar247 et les spécialistes du dessin qui travaillent sur votre commande voient les fichiers de numérisation, le libellé de la propriété et vos notes de commande, et sont tenus à des obligations de confidentialité. Nous faisons aussi appel à des prestataires de services qui traitent des données pour notre compte : hébergement infonuagique et stockage d'objets, un prestataire d'envoi de courriels et Stripe, notre prestataire de paiement. Tous sont liés par des conditions qui les obligent à protéger vos renseignements à un niveau au moins équivalent à celui prévu par la présente politique. Nos prestataires d'hébergement, de stockage et d'envoi de courriels agissent uniquement sur nos instructions et n'utilisent vos renseignements que pour le service qu'ils nous fournissent. Stripe n'agit pas uniquement sur nos instructions : en tant qu'entreprise de paiement, elle utilise aussi les données de paiement à ses propres fins, notamment la prévention de la fraude et la sécurité, le respect de ses propres obligations légales et l'amélioration de ses services. Son formulaire de paiement lui transmet des données sur la manière dont le formulaire est utilisé ainsi que votre adresse réseau (IP) et des caractéristiques de base de votre appareil, telles que le modèle, la version du système, la taille de l'écran, la langue et le fuseau horaire et des identifiants générés par Stripe, et, dans l'application, la version de l'application et l'identifiant qu'iOS attribue aux applications du même développeur (voyez \"Identifiant de l'appareil\") ; Stripe utilise ces données à ses propres fins d'analyse et de prévention de la fraude. Stripe traite tout cela selon sa propre politique de confidentialité. Lorsque vous payez par carte, que ce soit dans l'application ou sur notre page de paiement, vous transmettez les données de votre carte directement à Stripe : elles ne transitent jamais par nos serveurs et nous ne voyons jamais votre numéro de carte complet. Lorsque l'application prépare le formulaire de carte de Stripe pour une commande, nous communiquons à Stripe votre nom, votre adresse courriel, des identifiants de votre compte et de votre commande, le numéro de commande et le montant à payer, que vous payiez ensuite par ce formulaire ou non ; lorsque l'application propose ce formulaire, elle le prépare automatiquement dès que vous touchez \"Passer la commande\" pour une commande qui n'est pas gratuite, puis chaque fois que vous touchez un bouton pour la payer. Dès ce moment, Stripe conserve votre nom et votre adresse courriel dans une fiche client sous notre compte Stripe, avec toute carte que vous choisissez d'enregistrer dans l'application ; nous demandons à Stripe de supprimer cette fiche lorsque vous supprimez votre compte. Nous recevons en retour le résultat et le montant de tout paiement. Dans notre compte Stripe, nous pouvons aussi voir la fiche que Stripe conserve au sujet de la carte — notamment sa marque, ses quatre derniers chiffres, sa date d'expiration, son type et son pays, ainsi que le code postal si le formulaire vous le demande — et l'évaluation du risque de fraude que Stripe fait du paiement ; pour les paiements faits dans l'application, nous ne conservons ni cette fiche ni cette évaluation dans nos propres systèmes. Notre page de paiement fait partie de notre site web, cedar247.com, et fonctionne avec son logiciel de boutique : elle peut transmettre à Stripe davantage de renseignements vous concernant que l'application — par exemple ceux d'un compte cedar247.com auquel vous êtes connecté, comme son adresse de facturation — et le site peut conserver des traces de ces paiements, comme la marque, les quatre derniers chiffres et la date d'expiration d'une carte que vous enregistrez dans ce compte. Si vous choisissez un autre mode de paiement proposé sur notre page de paiement, le prestataire de ce mode de paiement traite vos renseignements selon sa propre politique de confidentialité. Si vous autorisez les notifications, elles parviennent à votre appareil par le service de notifications poussées d'Apple : Apple reçoit le jeton de notification et le contenu de chaque notification, qui indique le libellé de la propriété ou le numéro de commande (voyez \"Notifications\"), et les traite selon sa propre politique de confidentialité. Nous pouvons divulguer des données si la loi l'exige, ou pour faire valoir ou défendre des droits en justice. Nous ne vendons pas de renseignements personnels et nous ne les communiquons pas à des courtiers en données."),
        ...
        ("Combien de temps nous les conservons",
         "Sur votre téléphone, les numérisations restent jusqu'à ce que vous les supprimiez — rien n'est supprimé automatiquement au bout d'un délai. Supprimer une propriété dans l'application supprime du téléphone les numérisations de cette propriété, et cela ne peut pas être annulé ; toute copie déjà téléversée chez nous — pour une commande, ou lorsque vous avez ouvert le formulaire de commande depuis cette numérisation sans commander — n'est pas concernée. Chaque numérisation est volumineuse ; supprimez donc les propriétés dont vous n'avez plus besoin pour récupérer de l'espace. De notre côté, nous conservons actuellement les fichiers de numérisation, les pièces jointes de commande et les dossiers de commande aussi longtemps qu'ils sont nécessaires à la commande et à ses registres ; un plan livré peut revenir pour révision, et les dossiers de commande sont exigés à des fins fiscales et comptables. Une numérisation téléversée lorsque vous avez ouvert le formulaire de commande sans commander, ou dont la commande a été annulée avant d'être payée, reste jusqu'à ce que vous supprimiez votre compte ou nous demandiez de la retirer. Nous n'effectuons pas aujourd'hui de purge automatique de ces fichiers. Les plans terminés restent disponibles en téléchargement pendant au moins 365 jours à compter de la livraison ; passé ce délai, nous pouvons les supprimer — conservez donc votre propre copie. Une partie de ces données survit au compte lui-même — voyez \"Suppression de votre compte\" ci-dessous. Si vous voulez que vos fichiers de numérisation soient supprimés plus tôt, supprimez votre compte ; pour les fichiers d'une commande encore en cours, comme pour les pièces jointes de commande, écrivez-nous par courriel et nous les supprimerons une fois la commande clôturée."),
        ("Suppression de votre compte",
         "Dans Compte, l'option Supprimer le compte supprime votre compte et, de notre stockage, vos fichiers de numérisation ainsi que les photos de visite virtuelle que vous avez téléversées dans l'application, sauf ceux décrits ci-dessous. Nous demandons aussi à Stripe de supprimer toute fiche client que l'application a créée pour vous chez Stripe, avec toute carte que vous avez enregistrée dans l'application ; Stripe conserve ses propres registres des paiements eux-mêmes, que nous pouvons encore voir dans notre compte Stripe. La suppression du compte ne touche pas une carte enregistrée sur notre page de paiement : une carte enregistrée dans un compte cedar247.com peut être retirée depuis ce compte, ou par nous si vous nous écrivez, et une carte enregistrée avec Link de Stripe reste dans votre propre compte Link chez Stripe, que vous gérez vous-même. Certaines données survivent à la suppression, principalement les suivantes. Nos registres commerciaux des commandes que vous avez passées restent, ainsi que la commande que notre site web crée pour chaque commande payante et dans laquelle son paiement est enregistré ; ils contiennent ce que vous nous avez fourni pour chaque commande, par exemple votre nom et votre adresse courriel, l'adresse ou le libellé de la propriété, les forfaits, options et code promotionnel, vos notes, vos demandes de révision et vos messages, les noms et les mesures de vos numérisations et les références de paiement, parce que le droit fiscal exige ce registre et qu'un plan livré peut encore revenir pour révision. Une commande passée dans l'application qui attend encore son paiement est d'abord annulée, de sorte que ses fichiers de numérisation et ses photos de visite sont supprimés comme les autres, sauf si elle ne peut pas être annulée à ce moment-là, par exemple parce qu'un paiement pour celle-ci est en cours. Les fichiers de numérisation et les photos de visite virtuelle de toute autre commande encore ouverte au moment de la suppression, payée ou non, restent pour permettre d'achever le travail, et ils ne sont pas supprimés automatiquement à sa clôture. Les autres fichiers que vous avez joints vous-même, par exemple un logo ou un PDF annoté, restent, que vous les ayez envoyés avec une commande ou non. Les plans que nous avons livrés, et toute page de visite virtuelle que nous avons publiée, restent accessibles par leurs liens ; une page de visite perd les photos que vous avez téléversées dans l'application et toute image de plan fournie avec vos fichiers de numérisation, sauf si sa commande est encore ouverte, mais garde son titre (souvent l'adresse de la propriété), les noms des pièces et des étages, les plans que notre équipe a dessinés ou téléversés, les photos que vous nous avez envoyées et que notre équipe y a ajoutées pour vous, et tout autre ajout de notre équipe. La trace décrite sous \"Identifiant de l'appareil\" reste aussi. Des copies de notre base de données conservent également la fiche de votre compte, y compris l'empreinte cryptographique de votre mot de passe, et les fiches de vos numérisations : les sauvegardes quotidiennes, supprimées environ 60 jours plus tard, et les copies que nous faisons à la main de temps à autre pour mettre les données en sécurité ou pour tester des modifications du service, conservées aussi longtemps que nous en avons besoin. Nous faisons aussi à la main des extraits plus limités — des renseignements de compte et de commande, ainsi que les fiches de vos numérisations avec l'étiquette que vous avez saisie, le type, l'état et les dates — pour comprendre comment le service est utilisé, et nous les conservons aussi longtemps que nous en avons besoin. Nos journaux techniques ordinaires (voyez \"Journaux techniques\") restent aussi longtemps que nous les conservons pour faire fonctionner le service. Écrivez à \(contactEmail) si vous souhaitez que des fichiers, des pages ou des renseignements de commande mentionnés ci-dessus soient supprimés — par exemple les fichiers d'une commande une fois celle-ci clôturée, une pièce jointe que vous n'avez jamais envoyée ou une page de visite — et nous le ferons partout où la loi nous le permet ; les copies de la base de données, ces extraits et les journaux techniques ne sont pas modifiés au cas par cas, et les registres propres à Stripe relèvent de sa politique de confidentialité. La suppression du compte lui-même ne peut pas être annulée."),
        ("Vos droits",
...
extension LegalDoc {
    static let termsSectionsFR: [(String, String)] = [
        ...
        ("Paiement",
         "Les commandes payantes sont réglées soit par carte par l'intermédiaire de Stripe, notre prestataire de paiement, dans l'application lorsque celle-ci le propose ou, dans votre navigateur, sur notre page de paiement, soit par tout autre mode de paiement proposé par cette page. Le montant débité est le total affiché sur le formulaire de paiement, en dollars américains, après application de tout code promotionnel. Si vous choisissez d'enregistrer une carte dans l'application, elle est conservée par Stripe, non par nous, et n'est utilisée que lorsque vous confirmez un paiement ultérieur ; vous pouvez la retirer dans le formulaire de carte de l'application chaque fois qu'il s'ouvre pour un paiement ultérieur, ou à tout moment en nous écrivant ou en supprimant votre compte, ce qui demande à Stripe de la supprimer. La production commence une fois le paiement confirmé, sauf si la commande est couverte par le quota de commandes gratuites, auquel cas elle commence immédiatement. Si un paiement est annulé ou fait l'objet d'une rétrofacturation, nous pouvons suspendre la commande ou retenir les livrables jusqu'à ce que la situation soit résolue."),
...
extension LegalDoc {
    static let eulaSectionsFR: [(String, String)] = [
        ...
        ("Services de tiers",
         "L'application repose sur des services que nous ne contrôlons pas, notamment les cadres logiciels de la plateforme d'Apple, nos fournisseurs d'hébergement infonuagique et de stockage, et Stripe, notre prestataire de paiement, qui fournit le formulaire de paiement par carte. Leurs conditions s'appliquent à leur partie du service, et nous ne sommes pas responsables de la manière dont ces tiers exercent leurs activités. Vous vous engagez à respecter toutes les conditions de tiers applicables à votre utilisation de l'application."),
        ("Absence de garantie",
