...
enum LegalDoc: String, CaseIterable, Identifiable {
    ...
    /// AND their scan rows, so "Why we use your data" now names understanding how the service is used, the
    /// hand-made copies in "Deleting your account" name that purpose too, and the manifest + App Privacy table
    /// give six types Apple's Analytics purpose (email, name, purchase history, account id, other user
    /// content = the scan label, other usage data). Round 22: the scan name is a floor label, not the
