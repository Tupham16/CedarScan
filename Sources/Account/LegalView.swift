...
enum LegalDoc: String, CaseIterable, Identifiable {
    ...
    /// the in-app channel is switched on or off. `date`: 2026-09-18.)
...
extension LegalDoc {
    static let privacySections: [(String, String)] = [
        ...
        ("Who sees your data",
         "The Cedar247 production team and the drafting specialists working on your order see the scan files, the property label and your order notes, under confidentiality obligations. We also use service providers who process data on our behalf: cloud hosting and object storage, an email delivery provider, and Stripe, our payment provider. They act on our instructions only, and we only work with providers whose terms require them to protect your data to at least the standard set out in this policy and to use it only for the service they provide to us; Stripe, as a payment company, additionally uses payment data for its own fraud prevention under its own privacy policy. When you pay by card, whether in the app or on our payment page, you give your card details to Stripe directly: they never pass through our servers and we never see your full card number. We pass Stripe your name, email address, order number and the amount due so that it can take the payment and keep any card you choose to save under your account, and we receive back only whether the payment succeeded, the amount, and the card brand and last four digits. If you choose another payment method offered on our payment page, that provider handles your details under its own policy. We may disclose data if the law requires it, or to establish or defend legal claims. We do not sell personal data and we do not share it with data brokers."),
...
extension LegalDoc {
    static let privacySectionsFR: [(String, String)] = [
        ...
        ("Renseignements que vous nous fournissez",
         "Renseignements sur le compte : votre nom, votre adresse courriel et un mot de passe (conservé uniquement sous forme d'empreinte cryptographique). Renseignements sur la propriété : l'adresse ou le libellé que vous saisissez pour chaque habitation, afin que l'équipe de dessin sache à quel bâtiment un plan se rapporte. Renseignements sur la commande : les forfaits et options que vous choisissez, vos préférences de dessin, les notes que vous rédigez, les codes promotionnels et les fichiers que vous joignez, par exemple un logo ou un PDF annoté. Renseignements de paiement : lorsque vous payez par carte, vous saisissez les données de votre carte dans un formulaire de paiement fourni par Stripe, et elles sont transmises à Stripe, non à nous — voyez \"Qui voit vos renseignements\"."),
