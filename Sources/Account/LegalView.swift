...
enum LegalDoc: String, CaseIterable, Identifiable {
    ...
    /// 20/09: owner decision (E) — the hand-run customer report analyses app accounts, so "Why we use your
    /// data" now names understanding how the service is used, and the manifest + App Privacy table give
    /// email, name, purchase history and the account id Apple's Analytics purpose.)
...
extension LegalDoc {
    static let privacySectionsFR: [(String, String)] = [
        ...
        ("Pourquoi nous utilisons vos renseignements",
         "Pour exécuter notre contrat avec vous : produire et livrer les plans que vous commandez, encaisser le paiement et vous assister ensuite. Pour nos intérêts légitimes : maintenir la sécurité et le fonctionnement du service, tester ses modifications, prévenir les abus des promotions, améliorer la précision des consignes de capture et comprendre comment le service est utilisé, par exemple combien de personnes s'inscrivent, commandent ou s'arrêtent avant de commander. Pour respecter nos obligations légales : fiscalité, comptabilité et tenue de registres. Lorsque la loi exige un consentement — par exemple pour la localisation de l'appareil — nous le demandons, et vous pouvez le refuser ou le retirer dans les réglages de votre appareil."),
