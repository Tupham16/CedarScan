...
enum LegalDoc: String, CaseIterable, Identifiable {
    ...
    /// Stripe account but do not store.)
...
extension LegalDoc {
    static let privacySections: [(String, String)] = [
        ...
        ("Who sees your data",
         "The Cedar247 production team and the drafting specialists working on your order see the scan files, the property label and your order notes, under confidentiality obligations. We also use service providers who process data on our behalf: cloud hosting and object storage, an email delivery provider, and Stripe, our payment provider. They act on our instructions only, and we only work with providers whose terms require them to protect your data to at least the standard set out in this policy and to use it only for the service they provide to us. Stripe is the exception in one respect: as a payment company it also uses payment data for its own fraud prevention, and its payment form reports to Stripe how the form is used, together with basic device details — device model, system version, screen size, language, time zone and a device identifier Stripe generates — for Stripe's analytics and fraud prevention, all under Stripe's own privacy policy. When you pay by card, whether in the app or on our payment page, you give your card details to Stripe directly: they never pass through our servers and we never see your full card number. We pass Stripe your name, email address, an identifier for your account, the order number and the amount due so that it can take the payment and keep any card you choose to save under your account. We receive back the result and the amount of the payment; the record Stripe keeps of the card — brand, last four digits, expiry date, card country and the postal code if the form asks for one — is visible to us in our Stripe account, but we do not store it in our own systems. If you choose another payment method offered on our payment page, that provider handles your details under its own policy. We may disclose data if the law requires it, or to establish or defend legal claims. We do not sell personal data and we do not share it with data brokers."),
